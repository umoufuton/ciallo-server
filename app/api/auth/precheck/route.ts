...
export async function POST(request: Request) {
  ...
  const existing = await findPilotByUsername(username);
  if (existing) {
    return Response.json({
      exists: true,
      hasPassword: Boolean(existing.password_hash),
      hasVerifiedEmail: Boolean(existing.email && existing.email_verified_at),
      source: "local",
      pilot: toPublicPilot(existing),
    });
  }
